...
-- Ensure the active contact constraint exists for pre-existing tables
do $$
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'driver_profiles_active_requires_contact'
      and conrelid = 'public.driver_profiles'::regclass
  ) then
    alter table public.driver_profiles
      add constraint driver_profiles_active_requires_contact
      check (status <> 'active' or (full_name is not null and phone is not null));
  end if;
end $$;
...
-- NOTE: Keep policy names stable for audits.
...
-- NOTE: Prevent suspended drivers from updates.
...
-- NOTE: Admins are scoped via profiles.is_admin.
